        ...
        public async Task LoadStatistics()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < 7; i++)
                {
                    ...
                    presentCounts[i] = last7.Count(a => a.Date.Date == d.Date && (a.Status == "Present" || a.Status == "Late"));
                }
                ...
                System.Windows.Application.Current?.Dispatcher.Invoke(() =>
                {
                    ...
                    foreach (var dept in depts)
                    {
                        ...
                        if (count > 0)
                            DepartmentDistributionSeries.Add(new PieSeries { Title = dept.DeptName ?? "N/A", Values = new ChartValues<int> { count } });
                    }
